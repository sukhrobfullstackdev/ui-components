...
export type SecuredByMagicProps = {
  enableLink?: boolean;
  label?: string;
  onLinkClick?: () => void;
};

const SecuredByMagic = ({ enableLink = true, label = 'Secured by', onLinkClick }: SecuredByMagicProps) => {
  const gradientFill = enableLink ? { fill: 'url("#logo-gradient")' } : {};

  const logoSVG = (
    <svg width={62} height={23} viewBox="0 0 83 31" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="logo-gradient" x1="-100%" y1="0%" x2="100%" y2="0%" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#402FC3" />
          <stop offset="33%" stopColor="#C970FF" />
          <stop offset="67%" stopColor="#C970FF" />
          <stop offset="100%" stopColor="#402FC3" />
    ...
    >
      <p
        className={css({
          color: 'ink.50',
          _dark: { color: 'ink.70' },
          _groupHover: enableLink ? { color: 'text.tertiary' } : {},
          fontSize: 'xs',
          mr: 2,
        })}
      >
  ...
  return (
    <div className={css({ cursor: enableLink ? 'pointer' : 'default' }) + ' group'}>
      {enableLink && (
        <a
          onClick={onLinkClick}
          tabIndex={0}
